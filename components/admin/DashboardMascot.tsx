...
export default function DashboardMascot() {
  ...
  // Listen for chat events from AdminChatWidget
  useEffect(() => {
    const handler = (e: Event) => {
      const type = (e as CustomEvent).detail?.type
      if (type === 'peeking') startPeeking()
      else if (type === 'stop-peeking') stopPeeking()
      else if (type === 'thinking') startThinking()
      else if (type === 'replying') startReplyBubbles()
      else if (type === 'done') stopReplyBubbles()
    }
    window.addEventListener('mascot-chat', handler)
    return () => window.removeEventListener('mascot-chat', handler)
  }, [startPeeking, stopPeeking, startThinking, startReplyBubbles, stopReplyBubbles])
