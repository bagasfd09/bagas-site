...
async function fetchSearchConsoleData(days: number) {
  ...
  const now = new Date()
  const endDate = new Date(now.getTime() - 2 * 24 * 60 * 60 * 1000) // GSC data has ~2 day delay
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ configured: false, error: 'Failed to fetch Search Console data' })
  }
}
